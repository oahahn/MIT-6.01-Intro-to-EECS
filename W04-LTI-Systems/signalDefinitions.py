...
class StepSignal(Signal):
    """Has value 0 at any time n < 0 and a value of 1 otherwise"""
    def sample(self, n):
        if n < 0:
            return 0
        else:
...
class SummedSignal(Signal):
    """Adds the value of two signals together at each time step"""
...
class ScaledSignal(Signal):
    """Scales the value of a signal by a factor c at each time step"""
...
class R(Signal):
    """Delays a signal by 1 time step"""
    def __init__(self, s):
        self.s = s
    def sample(self, n):
        self.s.sample(n - 1)
...
class Rn(Signal):
    """Delays a signal by k time steps"""
    def __init__(self, s, k):
        self.s = s
        self.k = k
    def sample(self, n):
        self.s.sample(n - self.k)

def polyR(signal, polynomial):
    """
    Args:
        signal: an instance of the Signal class
        polynomial: an instance of the Polynomial class

    Returns:
        newSignal: a new signal that is signal transformed by polynomial in R
    """
    # Make a list of signals to add
    signals = []
    # The coefficients are originally in descending order in powers of R
    polynomial.reverse()
    for i in range(len(polynomial)):
        # Delay signal by i
        d = Rn(signal, i)
        # Multiply this by the coefficient at i
        s = ScaledSignal(d, polynomial[i])
        signals.append(s)
    newSignal = ConstantSignal(0)
    # Add the signals into one compact signal
